...
<header>
    ...
    <ul>
		...
		<div class="container">
			...
			<font size="4" color="white">Pending Approval!
				...
				<table class = "container1">
				    ...
				    <?php
						...
						if ($conn->connect_error) {
							...
							$sql = "SELECT Post_ID, Title, Description, Rent, curdate, Status FROM residential_house" ;
								...
								while($row = $result->fetch_assoc()) {
									...
									if( strcmp( $vr, "UNVERIFIED" ) == 0 ){
										...
										<tr>
									      ...
									      <td><?php echo $c; ?></td>
									      <td><?php echo $d; ?></td>
									      <td><form method = "post" ><input name = <?php echo $formid  ?> type="SUBMIT" value="verify" style="width:100%"></form></td>
									    </tr>
							...
							$sql = "SELECT Post_ID, Title, Description, Rent, curdate, Status FROM commercial_space" ;
							$result = $conn->query($sql);
							$formid = "form";
							$brr = array(100);
							$cnt = $arrid;
				...
				if ($conn->connect_error) {
					...
					for( $i = 0; $i < $arrid; $i++ ){
						...
						if( isset($_POST[$tmp] )){
							...
							$sql = "UPDATE residential_house SET Status='VERIFIED' WHERE Post_ID= $arr[$i]";
					...
					for( $i = $arrid; $i < $cnt ; $i++ , $ct++ ){
						...
						if( isset($_POST[$tmp] )){
							#echo  $brr[$ct] . "</br>";
							$sql = "UPDATE commercial_space SET Status='VERIFIED' WHERE Post_ID= $brr[$ct]";
							if ($conn->query($sql) === TRUE) {
								echo "Record updated successfully";
							} else {
								 echo "Error updating record: " . $conn->error;
